...
import { FaWhatsapp, FaFacebook, FaStar, FaHeart, FaPhone, FaShare } from "react-icons/fa";
                            ...
                            {product.images.map((img, idx) => (
                                <div 
                                    ...
                                    className={`cursor-pointer border-2 rounded-lg overflow-hidden bg-gray-50 p-2 transition-all ${
                                        selectedImage === idx ? "border-primary" : "border-transparent hover:border-primary"
                                    }`}
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center space-x-3">
                            {product.discountPrice ? (
                                <>
                                    <span className="text-3xl font-bold text-blue-500 dark:text-yellow-200">৳{product.discountPrice.toLocaleString()}</span>
                                    <span className="line-through text-gray-400 text-xl">৳{product.price.toLocaleString()}</span>
                    ...
                    <div className="bg-base-200 p-4 rounded-lg">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={product.stock.available > 0 ? "text-success font-semibold" : "text-error font-semibold"}>
                                ...
                                {product.colors.map((color, index) => (
                                    <div key={index} className="flex flex-col items-center">
                                        <div 
                                            ...
                                            style={{ backgroundColor: color.toLowerCase() === 'silver' ? '#c0c0c0' : color.toLowerCase() }}
                                            title={color}
                                        ></div>
                    ...
                    <div className="rounded-2xl bg-base-100 p-6 shadow-sm">
                        ...
                        <div className="flex flex-col sm:flex-row justify-center items-center gap-6 mt-6">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <a href="tel:01712076011" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-base-100 dark:bg-base-300 shadow-sm hover:shadow-md transition-shadow">
                                    <FaPhone className="text-green-500" />
                                    <span className="font-medium">01712076011</span>
                                </a>
                                <a href="tel:01868944455" className="flex items-center gap-2 px-4 py-2 rounded-lg bg-base-100 dark:bg-base-300 shadow-sm hover:shadow-md transition-shadow">
                                    <FaPhone className="text-green-500" />
                    ...
                    <div className="bg-base-100 rounded-2xl shadow-sm overflow-hidden">
                        {product.specifications.map((spec, index) => (
                            <div key={index} className="border-b last:border-b-0">
                                <h3 className="font-semibold text-lg p-4 bg-base-200">{spec.key}</h3>
                                <div className="p-4">
                                    {typeof spec.value === 'object' ? (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {Object.entries(spec.value).map(([key, value], i) => (
                                                <div key={i} className="flex">
                                                    <span className="font-medium w-1/2 capitalize">{key}:</span>
